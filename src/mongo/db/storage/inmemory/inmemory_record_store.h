 ...
 * conflict exception if the IMWriteConflictException failpoint is enabled. This is only checked
 * on cursor methods that make modifications.
 */
#define WT_OP_CHECK(x) (((MONGO_FAIL_POINT(IMWriteConflictException))) ? (WT_ROLLBACK) : (x))

/**
 * Identical to WT_OP_CHECK except this is checked on cursor seeks/advancement.
 */
#define WT_READ_CHECK(x) \
    (((MONGO_FAIL_POINT(IMWriteConflictExceptionForReads))) ? (WT_ROLLBACK) : (x))
...
class InMemoryRecoveryUnit;
class InMemorySizeStorer;

extern const std::string kInMemoryEngineName;
...
public:
     ...
     * The document 'options' is typically obtained from the 'inMemory' field of
     ...
     *     built-in defaults
     *     storageEngine.inMemory.configString in 'options'
     *     'extraStrings'
    ...
    InMemoryRecordStore(OperationContext* txn,
                          StringData ns,
                          StringData uri,
                          ...
                          InMemorySizeStorer* sizeStorer = nullptr);

    virtual ~InMemoryRecordStore();
    ...
    void setSizeStorer(InMemorySizeStorer* ss) {
        _sizeStorer = ss;
    }
...
private:
    ...
    static InMemoryRecoveryUnit* _getRecoveryUnit(OperationContext* txn);
    ...
    RecordData _getData(const InMemoryCursor& cursor) const;
    void _oplogSetStartHack(InMemoryRecoveryUnit* wru) const;
    void _oplogJournalThreadLoop(InMemorySessionCache* sessionCache);
    ...
    InMemorySizeStorer* _sizeStorer;  // not owned, can be NULL
...
MONGO_FP_FORWARD_DECLARE(IMWriteConflictException);
MONGO_FP_FORWARD_DECLARE(IMWriteConflictExceptionForReads);
...
MONGO_FP_FORWARD_DECLARE(IMPausePrimaryOplogDurabilityLoop);
}
