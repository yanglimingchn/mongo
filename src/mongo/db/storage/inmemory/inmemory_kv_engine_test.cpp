...
public:
    InMemoryKVHarnessHelper() : _dbpath("wt-kv-harness") {
        _engine.reset(new InMemoryKVEngine(
            kInMemoryEngineName, _dbpath.path(), "", 1, false, false, false));
    }
...
private:
    ...
    std::unique_ptr<InMemoryKVEngine> _engine;
};
