...
public:
    ...
    std::unique_ptr<SortedDataInterface> newSortedDataInterface(bool unique) final {
        ...
        StatusWith<std::string> result =
            InMemoryIndex::generateCreateString(kInMemoryEngineName, "", "", desc);
        ASSERT_OK(result.getStatus());

        string uri = "table:" + ns;
        invariantWTOK(InMemoryIndex::Create(&txn, uri, result.getValue()));

        if (unique)
            return stdx::make_unique<InMemoryIndexUnique>(&txn, uri, &desc);
        return stdx::make_unique<InMemoryIndexStandard>(&txn, uri, &desc);
    }

    std::unique_ptr<RecoveryUnit> newRecoveryUnit() final {
        return stdx::make_unique<InMemoryRecoveryUnit>(_sessionCache);
    }
...
TEST(InMemoryIndexTest, GenerateCreateStringEmptyDocument) {
    BSONObj spec = fromjson("{}");
    StatusWith<std::string> result = InMemoryIndex::parseIndexOptions(spec);
    ASSERT_OK(result.getStatus());
    ASSERT_EQ(result.getValue(), "");  // "," would also be valid.
}

TEST(InMemoryIndexTest, GenerateCreateStringUnknownField) {
...
TEST(InMemoryIndexTest, GenerateCreateStringNonStringConfig) {
...
TEST(InMemoryIndexTest, GenerateCreateStringEmptyConfigString) {
...
TEST(InMemoryIndexTest, GenerateCreateStringInvalidConfigStringOption) {
    BSONObj spec = fromjson("{configString: 'abc=def'}");
    ASSERT_EQ(InMemoryIndex::parseIndexOptions(spec), ErrorCodes::BadValue);
}

TEST(InMemoryIndexTest, GenerateCreateStringValidConfigStringOption) {
    BSONObj spec = fromjson("{configString: 'prefix_compression=true'}");
    ASSERT_EQ(InMemoryIndex::parseIndexOptions(spec), std::string("prefix_compression=true,"));
}
