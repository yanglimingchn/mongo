...
public:
    InMemorySizeStorer(WT_CONNECTION* conn, const std::string& storageUri);
    ~InMemorySizeStorer();

    void onCreate(InMemoryRecordStore* rs, long long nr, long long ds);
    void onDestroy(InMemoryRecordStore* rs);
...
private:
    ...
    struct Entry {
        ...
        long long dataSize;
        bool dirty;
        InMemoryRecordStore* rs;  // not owned
    };
    ...
    const InMemorySession _session;
