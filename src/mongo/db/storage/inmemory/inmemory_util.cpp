...
Status imRCToStatus_slow(int retCode, const char* prefix) {
    if (retCode == 0)
        return Status::OK();
    ...
    fassert(58559, retCode != WT_PANIC);
...
void InMemoryUtil::fetchTypeAndSourceURI(OperationContext* opCtx,
    ...
    InMemoryConfigParser parser(colgroupResult.getValue());
...
StatusWith<std::string> InMemoryUtil::getMetadata(OperationContext* opCtx, StringData uri) {
    invariant(opCtx);
    InMemoryCursor curwrap("metadata:create", InMemorySession::kMetadataTableId, false, opCtx);
...
Status InMemoryUtil::getApplicationMetadata(OperationContext* opCtx,
                                              StringData uri,
                                              BSONObjBuilder* bob) {
    ...
    InMemoryConfigParser topParser(metadataResult.getValue());
    ...
    InMemoryConfigParser parser(appMetadata);
...
StatusWith<BSONObj> InMemoryUtil::getApplicationMetadata(OperationContext* opCtx,
                                                           StringData uri) {
    BSONObjBuilder bob;
...
Status InMemoryUtil::checkApplicationMetadataFormatVersion(OperationContext* opCtx,
                                                             StringData uri,
    ...
    InMemoryConfigParser topParser(result.getValue());
    ...
    InMemoryConfigParser parser(metadata);
    ...
    LOG(2) << "InMemoryUtil::checkApplicationMetadataFormatVersion "
...
// static
Status InMemoryUtil::checkTableCreationOptions(const BSONElement& configElem) {
...
// static
StatusWith<uint64_t> InMemoryUtil::getStatisticsValue(WT_SESSION* session,
...
int64_t InMemoryUtil::getIdentSize(WT_SESSION* s, const std::string& uri) {
...
int mdb_handle_error(WT_EVENT_HANDLER* handler,
                     ...
                     const char* message) {
    try {
        error() << "InMemory (" << errorCode << ") " << message;
        fassert(58558, errorCode != WT_PANIC);
    } catch (...) {
        std::terminate();
    }
    return 0;
...
int mdb_handle_progress(WT_EVENT_HANDLER* handler,
                        ...
                        uint64_t progress) {
    try {
        log() << "InMemory progress " << operation << " " << progress;
    } catch (...) {
        std::terminate();
    }
...
WT_EVENT_HANDLER InMemoryUtil::defaultEventHandlers() {
...
InMemoryUtil::ErrorAccumulator::ErrorAccumulator(std::vector<std::string>* errors)
...
// static
int InMemoryUtil::ErrorAccumulator::onError(WT_EVENT_HANDLER* handler,
                                              WT_SESSION* session,
                                              int error,
                                              const char* message) {
    try {
...
int InMemoryUtil::verifyTable(OperationContext* txn,
                                const std::string& uri,
                                std::vector<std::string>* errors) {
    ErrorAccumulator eventHandler(errors);

    // Try to close as much as possible to avoid EBUSY errors.
    InMemoryRecoveryUnit::get(txn)->getSession(txn)->closeAllCursors(uri);
    InMemorySessionCache* sessionCache = InMemoryRecoveryUnit::get(txn)->getSessionCache();
    sessionCache->closeAllCursors(uri);

    // Open a new session with custom error handlers.
    WT_CONNECTION* conn = InMemoryRecoveryUnit::get(txn)->getSessionCache()->conn();
    WT_SESSION* session;
...
Status InMemoryUtil::exportTableToBSON(WT_SESSION* session,
                                         const std::string& uri,
                                         const std::string& config,
                                         BSONObjBuilder* bob) {
    invariant(session);
