...
class InMemoryRecoveryUnit final : public RecoveryUnit {
public:
    InMemoryRecoveryUnit(InMemorySessionCache* sc);
    ...
    InMemorySession* getSession(OperationContext* opCtx);
    ...
    InMemorySession* getSessionNoTxn(OperationContext* opCtx);
    ...
    static InMemoryRecoveryUnit* get(OperationContext* txn);
...
private:
    ...
    InMemorySessionCache* _sessionCache;  // not owned
    InMemorySession* _session;            // owned, but from pool
...
public:
    InMemoryCursor(const std::string& uri,
                     uint64_t tableID,
                     bool forRecordStore,
...
private:
    uint64_t _tableID;
    InMemoryRecoveryUnit* _ru;  // not owned
    InMemorySession* _session;
