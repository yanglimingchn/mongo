...
class InMemoryRecordStoreThread : public BackgroundJob {
public:
    InMemoryRecordStoreThread(const NamespaceString& ns)
    ...
    bool _deleteExcessDocuments() {
        ...
        try {
            ...
            InMemoryRecordStore* rs =
                checked_cast<InMemoryRecordStore*>(collection->getRecordStore());
            ...
            severe() << "error in InMemoryRecordStoreThread: " << e.what();
            fassertFailedNoTrace(!"error in InMemoryRecordStoreThread");
        } catch (...) {
            fassertFailedNoTrace(!"unknown error in InMemoryRecordStoreThread");
        }
        return true;
    }
...
// static
bool InMemoryKVEngine::initRsOplogBackgroundThread(StringData ns) {
    if (!NamespaceString::oplog(ns)) {
        return false;
    }

    if (storageGlobalParams.repair) {
        LOG(1) << "not starting InMemoryRecordStoreThread for " << ns
    ...
    if (_backgroundThreadNamespaces.count(nss)) {
        log() << "InMemoryRecordStoreThread " << ns << " already started";
    } else {
        log() << "Starting InMemoryRecordStoreThread " << ns;
        BackgroundJob* backgroundThread = new InMemoryRecordStoreThread(nss);
        backgroundThread->go();
