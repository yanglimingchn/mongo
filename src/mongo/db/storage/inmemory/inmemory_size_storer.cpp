...
InMemorySizeStorer::InMemorySizeStorer(WT_CONNECTION* conn, const std::string& storageUri)
    : _session(conn) {
...
InMemorySizeStorer::~InMemorySizeStorer() {
...
void InMemorySizeStorer::_checkMagic() const {
    if (MONGO_likely(_magic == MAGIC))
        return;
    log() << "InMemorySizeStorer magic wrong: " << _magic;
    invariant(_magic == MAGIC);
}

void InMemorySizeStorer::onCreate(InMemoryRecordStore* rs,
...
void InMemorySizeStorer::onDestroy(InMemoryRecordStore* rs) {
    _checkMagic();
    stdx::lock_guard<stdx::mutex> lk(_entriesMutex);
    Entry& entry = _entries[rs->getURI()];
...
void InMemorySizeStorer::storeToCache(StringData uri, long long numRecords, long long dataSize) {
    _checkMagic();
    stdx::lock_guard<stdx::mutex> lk(_entriesMutex);
...
void InMemorySizeStorer::loadFromCache(StringData uri,
                                         long long* numRecords,
...
void InMemorySizeStorer::fillCache() {
    stdx::lock_guard<stdx::mutex> cursorLock(_cursorMutex);
    _checkMagic();
    ...
    Map m;
    {
        ...
        while ((cursorNextRet = _cursor->next(_cursor)) != WT_NOTFOUND) {
            ...
            LOG(2) << "InMemorySizeStorer::loadFrom " << uriKey << " -> " << data;
...
void InMemorySizeStorer::syncCache(bool syncToDisk) {
    stdx::lock_guard<stdx::mutex> cursorLock(_cursorMutex);
    _checkMagic();
    ...
    for (Map::iterator it = myMap.begin(); it != myMap.end(); ++it) {
        ...
        LOG(2) << "InMemorySizeStorer::storeInto " << uriKey << " -> " << data;

        InMemoryItem key(uriKey.c_str(), uriKey.size());
        InMemoryItem value(data.objdata(), data.objsize());
