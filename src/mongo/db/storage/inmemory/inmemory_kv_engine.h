...
class InMemoryKVEngine final : public KVEngine {
public:
    InMemoryKVEngine(const std::string& canonicalName,
    ...
    std::list<InMemoryCachedCursor> filterCursorsWithQueuedDrops(
...
private:
    ...
    std::unique_ptr<InMemorySessionCache> _sessionCache;
    std::string _canonicalName;
    std::string _path;

    std::unique_ptr<InMemorySizeStorer> _sizeStorer;
    std::string _sizeStorerUri;
    mutable ElapsedTracker _sizeStorerSyncTracker;

    bool _durable;
    bool _ephemeral;
    std::unique_ptr<InMemoryJournalFlusher> _journalFlusher;  // Depends on _sizeStorer
    ...
    std::unique_ptr<InMemorySession> _backupSession;
};
}
