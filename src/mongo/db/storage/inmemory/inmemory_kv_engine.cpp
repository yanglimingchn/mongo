...
class InMemoryKVEngine::InMemoryJournalFlusher : public BackgroundJob {
public:
    explicit InMemoryJournalFlusher(InMemorySessionCache* sessionCache)
...
private:
    InMemorySessionCache* _sessionCache;
...
TicketServerParameter openWriteTransactionParam(&openWriteTransaction,
                                                "inMemoryConcurrentWriteTransactions");

TicketHolder openReadTransaction(128);
TicketServerParameter openReadTransactionParam(&openReadTransaction,
                                               "inMemoryConcurrentReadTransactions");

}  // namespace

InMemoryKVEngine::InMemoryKVEngine(const std::string& canonicalName,
                                       ...
                                       bool ephemeral,
                                       bool repair)
    : _eventHandler(InMemoryUtil::defaultEventHandlers()),
    ...
    ss << inMemoryGlobalOptions.journalCompressor << "),";
    ss << "file_manager=(close_idle_time=100000),";  //~28 hours, will put better fix in 3.1.x
    ss << "statistics_log=(wait=" << inMemoryGlobalOptions.statisticsLogDelaySecs << "),";
    ss << InMemoryCustomizationHooks::get(getGlobalServiceContext())->getOpenConfig("system");
    ss << extraOpenOptions;
    if (!_durable) {
        ...
        if (boost::filesystem::exists(journalPath)) {
            ...
            if (ret == EINVAL) {
                fassertFailedNoTrace(58717);
            } else if (ret != 0) {
                Status s(wtRCToStatus(ret));
                msgassertedNoTrace(58718, s.reason());
            }
    ...
    if (ret == EINVAL) {
        fassertFailedNoTrace(58561);
    } else if (ret != 0) {
        Status s(wtRCToStatus(ret));
        msgassertedNoTrace(58595, s.reason());
    }

    _sessionCache.reset(new InMemorySessionCache(this));

    if (_durable) {
        _journalFlusher = stdx::make_unique<InMemoryJournalFlusher>(_sessionCache.get());
        _journalFlusher->go();
    }

    _sizeStorerUri = "table:sizeStorer";
    {
        InMemorySession session(_conn);
        if (repair && _hasUri(session.getSession(), _sizeStorerUri)) {
            log() << "Repairing size cache";
            fassertNoTrace(58577, _salvageIfNeeded(_sizeStorerUri.c_str()));
        }
        _sizeStorer.reset(new InMemorySizeStorer(_conn, _sizeStorerUri));
...
InMemoryKVEngine::~InMemoryKVEngine() {
    if (_conn) {
        cleanShutdown();
    }
...
void InMemoryKVEngine::appendGlobalStats(BSONObjBuilder& b) {
...
void InMemoryKVEngine::cleanShutdown() {
    log() << "InMemoryKVEngine shutting down";
    syncSizeInfo(true);
    if (_conn) {
...
// We want InMemory to leak memory for faster shutdown except when we are running tools to
...
Status InMemoryKVEngine::okToRename(OperationContext* opCtx,
                                      StringData fromNS,
                                      StringData toNS,
                                      StringData ident,
...
int64_t InMemoryKVEngine::getIdentSize(OperationContext* opCtx, StringData ident) {
    InMemorySession* session = InMemoryRecoveryUnit::get(opCtx)->getSession(opCtx);
    return InMemoryUtil::getIdentSize(session->getSession(), _uri(ident));
}

Status InMemoryKVEngine::repairIdent(OperationContext* opCtx, StringData ident) {
    InMemorySession* session = InMemoryRecoveryUnit::get(opCtx)->getSession(opCtx);
...
Status InMemoryKVEngine::_salvageIfNeeded(const char* uri) {
    // Using a side session to avoid transactional issues
    InMemorySession sessionWrapper(_conn);
...
int InMemoryKVEngine::flushAllFiles(bool sync) {
    LOG(1) << "InMemoryKVEngine::flushAllFiles";
    syncSizeInfo(true);
...
void InMemoryKVEngine::endBackup(OperationContext* txn) {
    _backupSession.reset();
}

void InMemoryKVEngine::syncSizeInfo(bool sync) const {
    if (!_sizeStorer)
        return;
...
RecoveryUnit* InMemoryKVEngine::newRecoveryUnit() {
    return new InMemoryRecoveryUnit(_sessionCache.get());
}

void InMemoryKVEngine::setRecordStoreExtraOptions(const std::string& options) {
    _rsOptions = options;
}

void InMemoryKVEngine::setSortedDataInterfaceExtraOptions(const std::string& options) {
    _indexOptions = options;
}

Status InMemoryKVEngine::createRecordStore(OperationContext* opCtx,
                                             StringData ns,
                                             StringData ident,
    ...
    StatusWith<std::string> result =
        InMemoryRecordStore::generateCreateString(_canonicalName, ns, options, _rsOptions);
    if (!result.isOK()) {
    ...
    string uri = _uri(ident);
    WT_SESSION* s = session.getSession();
    LOG(2) << "InMemoryKVEngine::createRecordStore uri: " << uri << " config: " << config;
    return wtRCToStatus(s->create(s, uri.c_str(), config.c_str()));
}

RecordStore* InMemoryKVEngine::getRecordStore(OperationContext* opCtx,
                                                StringData ns,
                                                StringData ident,
                                                const CollectionOptions& options) {
    if (options.capped) {
        return new InMemoryRecordStore(opCtx,
                                         ns,
                                         _uri(ident),
                                         _canonicalName,
                                         options.capped,
...
string InMemoryKVEngine::_uri(StringData ident) const {
    return string("table:") + ident.toString();
}

Status InMemoryKVEngine::createSortedDataInterface(OperationContext* opCtx,
                                                     StringData ident,
    ...
    // order to allow for unit testing InMemoryKVEngine::createSortedDataInterface().
    if (collection) {
    ...
    StatusWith<std::string> result = InMemoryIndex::generateCreateString(
    ...
    LOG(2) << "InMemoryKVEngine::createSortedDataInterface ident: " << ident
           << " config: " << config;
    return wtRCToStatus(InMemoryIndex::Create(opCtx, _uri(ident), config));
}

SortedDataInterface* InMemoryKVEngine::getSortedDataInterface(OperationContext* opCtx,
                                                                StringData ident,
                                                                const IndexDescriptor* desc) {
    if (desc->unique())
        return new InMemoryIndexUnique(opCtx, _uri(ident), desc);
    return new InMemoryIndexStandard(opCtx, _uri(ident), desc);
}

Status InMemoryKVEngine::dropIdent(OperationContext* opCtx, StringData ident) {
    _drop(ident);
    return Status::OK();
}

bool InMemoryKVEngine::_drop(StringData ident) {
    string uri = _uri(ident);
...
std::list<InMemoryCachedCursor> InMemoryKVEngine::filterCursorsWithQueuedDrops(
    std::list<InMemoryCachedCursor>* cache) {
    std::list<InMemoryCachedCursor> toDrop;
...
bool InMemoryKVEngine::haveDropsQueued() const {
...
void InMemoryKVEngine::dropSomeQueuedIdents() {
    int numInQueue;
...
bool InMemoryKVEngine::supportsDocLocking() const {
    return true;
}

bool InMemoryKVEngine::supportsDirectoryPerDB() const {
    return true;
}

bool InMemoryKVEngine::hasIdent(OperationContext* opCtx, StringData ident) const {
    return _hasUri(InMemoryRecoveryUnit::get(opCtx)->getSession(opCtx)->getSession(),
                   _uri(ident));
}

bool InMemoryKVEngine::_hasUri(WT_SESSION* session, const std::string& uri) const {
    // can't use InMemoryCursor since this is called from constructor.
    WT_CURSOR* c = NULL;
...
std::vector<std::string> InMemoryKVEngine::getAllIdents(OperationContext* opCtx) const {
    std::vector<std::string> all;
    InMemoryCursor cursor("metadata:", InMemorySession::kMetadataTableId, false, opCtx);
...
int InMemoryKVEngine::reconfigure(const char* str) {
    return _conn->reconfigure(_conn, str);
}

void InMemoryKVEngine::_checkIdentPath(StringData ident) {
    size_t start = 0;
    size_t idx;
...
void InMemoryKVEngine::setJournalListener(JournalListener* jl) {
