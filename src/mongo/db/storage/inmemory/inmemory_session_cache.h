...
#include "mongo/db/storage/inmemory/inmemory_snapshot_manager.h"
...
class InMemoryKVEngine;

class InMemoryCachedCursor {
public:
    InMemoryCachedCursor(uint64_t id, uint64_t gen, WT_CURSOR* cursor)
...
public:
     ...
     */
    InMemorySession(WT_CONNECTION* conn, uint64_t epoch = 0, uint64_t cursorEpoch = 0);
    ...
    void closeCursorsForQueuedDrops(uint64_t cursorEpoch, InMemoryKVEngine* engine);
...
private:
    friend class InMemorySessionCache;

    // The cursor cache is a list of pairs that contain an ID and cursor
    typedef std::list<InMemoryCachedCursor> CursorCache;

    // Used internally by InMemorySessionCache
...
/**
 *  This cache implements a shared pool of InMemory sessions with the goal to amortize the
 *  cost of session creation and destruction over multiple uses.
 */
class InMemorySessionCache {
public:
    InMemorySessionCache(InMemoryKVEngine* engine);
    InMemorySessionCache(WT_CONNECTION* conn);
     ...
     */
    void releaseSession(InMemorySession* session);
    ...
    InMemorySnapshotManager& snapshotManager() {
        return _snapshotManager;
    }
    const InMemorySnapshotManager& snapshotManager() const {
        return _snapshotManager;
    }
...
private:
    InMemoryKVEngine* _engine;  // not owned, might be NULL
    WT_CONNECTION* _conn;         // not owned
    InMemorySnapshotManager _snapshotManager;
    ...
    typedef std::vector<InMemorySession*> SessionCache;
