...
InMemoryServerStatusSection::InMemoryServerStatusSection(InMemoryKVEngine* engine)
    : ServerStatusSection(kInMemoryEngineName), _engine(engine) {}

bool InMemoryServerStatusSection::includeByDefault() const {
    return true;
}

BSONObj InMemoryServerStatusSection::generateSection(OperationContext* txn,
    ...
    // immediately.
    InMemorySession* session = InMemoryRecoveryUnit::get(txn)->getSessionNoTxn(txn);
    invariant(session);
    ...
    BSONObjBuilder bob;
    Status status = InMemoryUtil::exportTableToBSON(s, uri, "statistics=(fast)", &bob);
    if (!status.isOK()) {
    ...
    InMemoryKVEngine::appendGlobalStats(bob);
