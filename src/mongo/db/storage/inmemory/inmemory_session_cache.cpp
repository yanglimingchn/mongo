...
InMemorySession::InMemorySession(WT_CONNECTION* conn, uint64_t epoch, uint64_t cursorEpoch)
    : _epoch(epoch),
...
WT_CURSOR* InMemorySession::getCursor(const std::string& uri, uint64_t id, bool forRecordStore) {
...
void InMemorySession::releaseCursor(uint64_t id, WT_CURSOR* cursor) {
    invariant(_session);
    invariant(cursor);
    _cursorsOut--;
    ...
    _cursors.push_front(InMemoryCachedCursor(id, _cursorGen++, cursor));
    _cursorsCached++;
...
void InMemorySession::closeAllCursors(const std::string& uri) {
    invariant(_session);
...
void InMemorySession::closeCursorsForQueuedDrops(uint64_t cursorEpoch,
                                                   InMemoryKVEngine* engine) {
    invariant(_session);
...
// static
uint64_t InMemorySession::genTableId() {
    return nextTableId.fetchAndAdd(1);
}

// -----------------------

InMemorySessionCache::InMemorySessionCache(InMemoryKVEngine* engine)
    : _engine(engine), _conn(engine->getConnection()), _snapshotManager(_conn), _shuttingDown(0) {}

InMemorySessionCache::InMemorySessionCache(WT_CONNECTION* conn)
...
void InMemorySessionCache::shuttingDown() {
...
void InMemorySessionCache::waitUntilDurable(bool forceCheckpoint) {
    ...
    if (forceCheckpoint && _engine->isDurable()) {
        InMemorySession* session = getSession();
...
void InMemorySessionCache::closeAllCursors(const std::string& uri) {
...
void InMemorySessionCache::closeCursorsForQueuedDrops() {
...
void InMemorySessionCache::closeAll() {
...
bool InMemorySessionCache::isEphemeral() {
    return _engine && _engine->isEphemeral();
}

InMemorySession* InMemorySessionCache::getSession() {
...

    {
        ...
        if (!_sessions.empty()) {
            ...
            InMemorySession* cachedSession = _sessions.back();
            _sessions.pop_back();
    ...
    return new InMemorySession(_conn, _epoch.load(), _cursorEpoch.load());
}

void InMemorySessionCache::releaseSession(InMemorySession* session) {
    invariant(session);
...
void InMemorySessionCache::setJournalListener(JournalListener* jl) {
    stdx::unique_lock<stdx::mutex> lk(_journalListenerMutex);
    _journalListener = jl;
}
}
