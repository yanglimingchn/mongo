...
public:
    InMemoryConnection(StringData dbpath, StringData extraStrings) : _conn(NULL) {
        std::stringstream ss;
        ss << "create,";
        ss << extraStrings;
...
class InMemoryUtilHarnessHelper {
public:
    InMemoryUtilHarnessHelper(StringData extraStrings)
        : _dbpath("wt_test"),
          _connection(_dbpath.path(), extraStrings),
          _sessionCache(_connection.getConnection()) {}


    InMemorySessionCache* getSessionCache() {
        return &_sessionCache;
    }

    OperationContext* newOperationContext() {
        return new OperationContextNoop(new InMemoryRecoveryUnit(getSessionCache()));
    }

private:
    unittest::TempDir _dbpath;
    InMemoryConnection _connection;
    InMemorySessionCache _sessionCache;
};

class InMemoryUtilMetadataTest : public mongo::unittest::Test {
public:
    virtual void setUp() {
        _harnessHelper.reset(new InMemoryUtilHarnessHelper(""));
...
protected:
    ...
    void createSession(const char* config) {
        WT_SESSION* wtSession =
            InMemoryRecoveryUnit::get(_opCtx.get())->getSession(_opCtx.get())->getSession();
        ASSERT_OK(wtRCToStatus(wtSession->create(wtSession, getURI(), config)));
    }

private:
    std::unique_ptr<InMemoryUtilHarnessHelper> _harnessHelper;
    std::unique_ptr<OperationContext> _opCtx;
};

TEST_F(InMemoryUtilMetadataTest, GetConfigurationStringInvalidURI) {
    StatusWith<std::string> result = InMemoryUtil::getMetadata(getOperationContext(), getURI());
    ASSERT_NOT_OK(result.getStatus());
    ASSERT_EQUALS(ErrorCodes::NoSuchKey, result.getStatus().code());
}

TEST_F(InMemoryUtilMetadataTest, GetConfigurationStringNull) {
...
TEST_F(InMemoryUtilMetadataTest, GetConfigurationStringSimple) {
...
TEST_F(InMemoryUtilMetadataTest, GetApplicationMetadataInvalidURI) {
    StatusWith<BSONObj> result =
        InMemoryUtil::getApplicationMetadata(getOperationContext(), getURI());
    ASSERT_NOT_OK(result.getStatus());
    ASSERT_EQUALS(ErrorCodes::NoSuchKey, result.getStatus().code());
}

TEST_F(InMemoryUtilMetadataTest, GetApplicationMetadataNull) {
    const char* config = NULL;
    createSession(config);
    StatusWith<BSONObj> result =
        InMemoryUtil::getApplicationMetadata(getOperationContext(), getURI());
...
TEST_F(InMemoryUtilMetadataTest, GetApplicationMetadataString) {
...
TEST_F(InMemoryUtilMetadataTest, GetApplicationMetadataDuplicateKeys) {
...
TEST_F(InMemoryUtilMetadataTest, GetApplicationMetadataTypes) {
    const char* config =
...
TEST_F(InMemoryUtilMetadataTest, CheckApplicationMetadataFormatVersionMissingKey) {
    createSession("app_metadata=(abc=123)");
    ASSERT_OK(InMemoryUtil::checkApplicationMetadataFormatVersion(
        getOperationContext(), getURI(), 1, 1));
    ASSERT_NOT_OK(InMemoryUtil::checkApplicationMetadataFormatVersion(
        getOperationContext(), getURI(), 2, 2));
}

TEST_F(InMemoryUtilMetadataTest, CheckApplicationMetadataFormatVersionString) {
    createSession("app_metadata=(formatVersion=\"bar\")");
    ASSERT_NOT_OK(InMemoryUtil::checkApplicationMetadataFormatVersion(
        getOperationContext(), getURI(), 1, 1));
}

TEST_F(InMemoryUtilMetadataTest, CheckApplicationMetadataFormatVersionNumber) {
...
TEST_F(InMemoryUtilMetadataTest, CheckApplicationMetadataFormatInvalidURI) {
    createSession("\"");
    Status result = InMemoryUtil::checkApplicationMetadataFormatVersion(
        getOperationContext(), getURI(), 0, 3);
    ASSERT_NOT_OK(result);
    ASSERT_EQUALS(ErrorCodes::FailedToParse, result.code());
}

TEST(InMemoryUtilTest, GetStatisticsValueMissingTable) {
    InMemoryUtilHarnessHelper harnessHelper("statistics=(all)");
    InMemoryRecoveryUnit recoveryUnit(harnessHelper.getSessionCache());
    InMemorySession* session = recoveryUnit.getSession(NULL);
    StatusWith<uint64_t> result =
        InMemoryUtil::getStatisticsValue(session->getSession(),
...
TEST(InMemoryUtilTest, GetStatisticsValueStatisticsDisabled) {
    InMemoryUtilHarnessHelper harnessHelper("statistics=(none)");
    InMemoryRecoveryUnit recoveryUnit(harnessHelper.getSessionCache());
    InMemorySession* session = recoveryUnit.getSession(NULL);
    WT_SESSION* wtSession = session->getSession();
    ASSERT_OK(wtRCToStatus(wtSession->create(wtSession, "table:mytable", NULL)));
    StatusWith<uint64_t> result = InMemoryUtil::getStatisticsValue(session->getSession(),
...
TEST(InMemoryUtilTest, GetStatisticsValueInvalidKey) {
    InMemoryUtilHarnessHelper harnessHelper("statistics=(all)");
    InMemoryRecoveryUnit recoveryUnit(harnessHelper.getSessionCache());
    InMemorySession* session = recoveryUnit.getSession(NULL);
    WT_SESSION* wtSession = session->getSession();
...
TEST(InMemoryUtilTest, GetStatisticsValueValidKey) {
    InMemoryUtilHarnessHelper harnessHelper("statistics=(all)");
    InMemoryRecoveryUnit recoveryUnit(harnessHelper.getSessionCache());
    InMemorySession* session = recoveryUnit.getSession(NULL);
    WT_SESSION* wtSession = session->getSession();
...
TEST(InMemoryUtilTest, GetStatisticsValueAsUInt8) {
    InMemoryUtilHarnessHelper harnessHelper("statistics=(all)");
    InMemoryRecoveryUnit recoveryUnit(harnessHelper.getSessionCache());
    InMemorySession* session = recoveryUnit.getSession(NULL);
    WT_SESSION* wtSession = session->getSession();
    ...
    StatusWith<uint8_t> resultUInt8 =
        InMemoryUtil::getStatisticsValueAs<uint8_t>(session->getSession(),
                                                      "statistics:table:mytable",
                                                      "statistics=(fast)",
                                                      WT_STAT_DSRC_ALLOCATION_SIZE);
    ...
    StatusWith<int16_t> resultInt16 =
        InMemoryUtil::getStatisticsValueAs<int16_t>(session->getSession(),
                                                      "statistics:table:mytable",
                                                      "statistics=(fast)",
                                                      WT_STAT_DSRC_ALLOCATION_SIZE,
