...
inline Status wtRCToStatus(int retCode, const char* prefix = NULL) {
    ...
    return imRCToStatus_slow(retCode, prefix);
}
...
struct InMemoryItem : public WT_ITEM {
    InMemoryItem(const void* d, size_t s) {
        data = d;
        size = s;
    }
    InMemoryItem(const std::string& str) {
        data = str.c_str();
        size = str.size();
    }
...
class InMemoryUtil {
    MONGO_DISALLOW_COPYING(InMemoryUtil);
...
class InMemoryConfigParser {
    MONGO_DISALLOW_COPYING(InMemoryConfigParser);

public:
    InMemoryConfigParser(StringData config) {
        invariantWTOK(
            wiredtiger_config_parser_open(NULL, config.rawData(), config.size(), &_parser));
    }

    InMemoryConfigParser(const WT_CONFIG_ITEM& nested) {
...
// static
template <typename ResultType>
StatusWith<ResultType> InMemoryUtil::getStatisticsValueAs(WT_SESSION* session,
                                                            const std::string& uri,
                                                            const std::string& config,
                                                            int statisticsKey) {
...
// static
template <typename ResultType>
ResultType InMemoryUtil::_castStatisticsValue(uint64_t statisticsValue) {
