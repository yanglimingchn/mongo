...
MONGO_FP_DECLARE(IMEmulateOutOfOrderNextIndexKey);
...
static const InMemoryItem emptyItem(NULL, 0);
...
Status checkKeySize(const BSONObj& key) {
    if (key.objsize() >= TempKeyMaxSize) {
        string msg = mongoutils::str::stream()
            << "InMemoryIndex::insert: key too large to index, failing " << ' ' << key.objsize()
            << ' ' << key;
...
Status InMemoryIndex::dupKeyError(const BSONObj& key) {
    StringBuilder sb;
...
// static
StatusWith<std::string> InMemoryIndex::parseIndexOptions(const BSONObj& options) {
    StringBuilder ss;
...
// static
StatusWith<std::string> InMemoryIndex::generateCreateString(const std::string& engineName,
    ...
    ss << "checksum=on,";
    if (inMemoryGlobalOptions.useIndexPrefixCompression) {
        ss << "prefix_compression=true,";
    }

    ss << "block_compressor=" << inMemoryGlobalOptions.indexBlockCompressor << ",";
    ss << InMemoryCustomizationHooks::get(getGlobalServiceContext())
...
InMemoryIndex::InMemoryIndex(OperationContext* ctx,
    ...
    if (!versionStatus.isOK()) {
        ...
        fassertFailedWithStatusNoTrace(58579, indexVersionStatus);
    }
}

Status InMemoryIndex::insert(OperationContext* txn,
                               const BSONObj& key,
                               const RecordId& id,
                               bool dupsAllowed) {
    ...
    InMemoryCursor curwrap(_uri, _tableId, false, txn);
...
void InMemoryIndex::unindex(OperationContext* txn,
                              const BSONObj& key,
                              const RecordId& id,
                              bool dupsAllowed) {
    invariant(id.isNormal());
...
void InMemoryIndex::fullValidate(OperationContext* txn,
                                   bool full,
                                   long long* numKeysOut,
                                   BSONObjBuilder* output) const {
    if (!InMemoryRecoveryUnit::get(txn)->getSessionCache()->isEphemeral()) {
        std::vector<std::string> errors;
        int err = InMemoryUtil::verifyTable(txn, _uri, output ? &errors : NULL);
        if (err == EBUSY) {
...
bool InMemoryIndex::appendCustomStats(OperationContext* txn,
                                        BSONObjBuilder* output,
                                        double scale) const {
    {
        BSONObjBuilder metadata(output->subobjStart("metadata"));
        Status status = InMemoryUtil::getApplicationMetadata(txn, uri(), &metadata);
        if (!status.isOK()) {
    ...
    WT_SESSION* s = session->getSession();
    Status status =
        InMemoryUtil::exportTableToBSON(s, "statistics:" + uri(), "statistics=(fast)", output);
    if (!status.isOK()) {
...
Status InMemoryIndex::dupKeyCheck(OperationContext* txn, const BSONObj& key, const RecordId& id) {
...
bool InMemoryIndex::isEmpty(OperationContext* txn) {
    InMemoryCursor curwrap(_uri, _tableId, false, txn);
    WT_CURSOR* c = curwrap.get();
    if (!c)
        return true;
...
Status InMemoryIndex::touch(OperationContext* txn) const {
    if (InMemoryRecoveryUnit::get(txn)->getSessionCache()->isEphemeral()) {
...
long long InMemoryIndex::getSpaceUsedBytes(OperationContext* txn) const {
    auto ru = InMemoryRecoveryUnit::get(txn);
    InMemorySession* session = ru->getSession(txn);
    ...
    if (ru->getSessionCache()->isEphemeral()) {
        ...
        auto getStats = [&](int key) -> int64_t {
            StatusWith<int64_t> result = InMemoryUtil::getStatisticsValueAs<int64_t>(
    ...
    return static_cast<long long>(InMemoryUtil::getIdentSize(session->getSession(), _uri));
}

bool InMemoryIndex::isDup(WT_CURSOR* c, const BSONObj& key, const RecordId& id) {
    invariant(unique());
    // First check whether the key exists.
    KeyString data(key, _ordering);
    InMemoryItem item(data.getBuffer(), data.getSize());
...
Status InMemoryIndex::initAsEmpty(OperationContext* txn) {
    // No-op
    return Status::OK();
}

/**
 * Base class for InMemoryIndex bulk builders.
 *
 * Manages the bulk cursor used by bulk builders.
 */
class InMemoryIndex::BulkBuilder : public SortedDataBuilderInterface {
public:
    BulkBuilder(InMemoryIndex* idx, OperationContext* txn)
        : _ordering(idx->_ordering),
          _txn(txn),
          _session(InMemoryRecoveryUnit::get(_txn)->getSessionCache()->getSession()),
          _cursor(openBulkCursor(idx)) {}

    ~BulkBuilder() {
        _cursor->close(_cursor);
        InMemoryRecoveryUnit::get(_txn)->getSessionCache()->releaseSession(_session);
    }

protected:
    WT_CURSOR* openBulkCursor(InMemoryIndex* idx) {
        // Open cursors can cause bulk open_cursor to fail with EBUSY.
        // TODO any other cases that could cause EBUSY?
        InMemorySession* outerSession = InMemoryRecoveryUnit::get(_txn)->getSession(_txn);
        ...
        warning() << "failed to create InMemory bulk cursor: " << wiredtiger_strerror(err);
    ...
    InMemorySession* const _session;
 ...
 */
class InMemoryIndex::StandardBulkBuilder : public BulkBuilder {
public:
    StandardBulkBuilder(InMemoryIndex* idx, OperationContext* txn)
    ...
    Status addKey(const BSONObj& key, const RecordId& id) {
        ...
        // Can't use InMemoryCursor since we aren't using the cache.
        InMemoryItem item(data.getBuffer(), data.getSize());
        _cursor->set_key(_cursor, item.Get());

        InMemoryItem valueItem = data.getTypeBits().isAllZeros()
            ? emptyItem
            : InMemoryItem(data.getTypeBits().getBuffer(), data.getTypeBits().getSize());
 ...
 * append data.
 */
class InMemoryIndex::UniqueBulkBuilder : public BulkBuilder {
public:
    UniqueBulkBuilder(InMemoryIndex* idx, OperationContext* txn, bool dupsAllowed)
...
private:
    Status doInsert() {
        ...
        InMemoryItem keyItem(_keyString.getBuffer(), _keyString.getSize());
        InMemoryItem valueItem(value.getBuffer(), value.getSize());
 ...
 */
class InMemoryIndexCursorBase : public SortedDataInterface::Cursor {
public:
    InMemoryIndexCursorBase(const InMemoryIndex& idx, OperationContext* txn, bool forward)
...
protected:
    ...
    bool seekWTCursor(const KeyString& query) {
        ...
        int cmp = -1;
        const InMemoryItem keyItem(query.getBuffer(), query.getSize());
    ...
    void updatePosition(bool inNext = false) {
        ...
        if (isForwardNextCall) {
            ...
            if (MONGO_FAIL_POINT(IMEmulateOutOfOrderNextIndexKey)) {
    ...
    const InMemoryIndex& _idx;  // not owned
    const bool _forward;
...
class InMemoryIndexStandardCursor final : public InMemoryIndexCursorBase {
public:
    InMemoryIndexStandardCursor(const InMemoryIndex& idx, OperationContext* txn, bool forward)
        : InMemoryIndexCursorBase(idx, txn, forward) {}
...
class InMemoryIndexUniqueCursor final : public InMemoryIndexCursorBase {
public:
    InMemoryIndexUniqueCursor(const InMemoryIndex& idx, OperationContext* txn, bool forward)
        : InMemoryIndexCursorBase(idx, txn, forward) {}
    ...
    boost::optional<IndexKeyEntry> seekExact(const BSONObj& key, RequestedInfo parts) override {
        ...
        const InMemoryItem keyItem(_query.getBuffer(), _query.getSize());
...
InMemoryIndexUnique::InMemoryIndexUnique(OperationContext* ctx,
                                             const std::string& uri,
                                             const IndexDescriptor* desc)
    : InMemoryIndex(ctx, uri, desc), _partial(desc->isPartial()) {}

std::unique_ptr<SortedDataInterface::Cursor> InMemoryIndexUnique::newCursor(OperationContext* txn,
                                                                              bool forward) const {
    return stdx::make_unique<InMemoryIndexUniqueCursor>(*this, txn, forward);
}

SortedDataBuilderInterface* InMemoryIndexUnique::getBulkBuilder(OperationContext* txn,
                                                                  bool dupsAllowed) {
    return new UniqueBulkBuilder(this, txn, dupsAllowed);
}

Status InMemoryIndexUnique::_insert(WT_CURSOR* c,
                                      const BSONObj& key,
                                      const RecordId& id,
                                      bool dupsAllowed) {
    const KeyString data(key, _ordering);
    InMemoryItem keyItem(data.getBuffer(), data.getSize());
    ...
    valueItem = InMemoryItem(value.getBuffer(), value.getSize());
    c->set_value(c, valueItem.Get());
    return wtRCToStatus(c->update(c));
}

void InMemoryIndexUnique::_unindex(WT_CURSOR* c,
                                     const BSONObj& key,
                                     const RecordId& id,
                                     bool dupsAllowed) {
    KeyString data(key, _ordering);
    ...
    InMemoryItem valueItem = InMemoryItem(newValue.getBuffer(), newValue.getSize());
    c->set_value(c, valueItem.Get());
...
InMemoryIndexStandard::InMemoryIndexStandard(OperationContext* ctx,
                                                 const std::string& uri,
                                                 const IndexDescriptor* desc)
    : InMemoryIndex(ctx, uri, desc) {}

std::unique_ptr<SortedDataInterface::Cursor> InMemoryIndexStandard::newCursor(
    OperationContext* txn, bool forward) const {
    return stdx::make_unique<InMemoryIndexStandardCursor>(*this, txn, forward);
}

SortedDataBuilderInterface* InMemoryIndexStandard::getBulkBuilder(OperationContext* txn,
                                                                    bool dupsAllowed) {
...
Status InMemoryIndexStandard::_insert(WT_CURSOR* c,
    ...
    InMemoryItem keyItem(key.getBuffer(), key.getSize());

    InMemoryItem valueItem = key.getTypeBits().isAllZeros()
        ? emptyItem
        : InMemoryItem(key.getTypeBits().getBuffer(), key.getTypeBits().getSize());
...
void InMemoryIndexStandard::_unindex(WT_CURSOR* c,
                                       const BSONObj& key,
                                       const RecordId& id,
                                       bool dupsAllowed) {
    invariant(dupsAllowed);
...
int inmemory_index_collator_customize(WT_COLLATOR* coll,
                             WT_SESSION* s,
                             const char* uri,
                             WT_CONFIG_ITEM* metadata,
                             WT_COLLATOR** collp) {
    fassertFailedWithStatusNoTrace(58580,
...
extern "C" MONGO_COMPILER_API_EXPORT int inmemory_index_collator_extension(WT_CONNECTION* conn,
                                                                  WT_CONFIG_ARG* cfg) {
    static WT_COLLATOR idx_static;

    idx_static.customize = inmemory_index_collator_customize;
