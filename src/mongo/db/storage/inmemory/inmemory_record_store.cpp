...
bool shouldUseOplogHack(OperationContext* opCtx, const std::string& uri) {
    StatusWith<BSONObj> appMetadata = InMemoryUtil::getApplicationMetadata(opCtx, uri);
    if (!appMetadata.isOK()) {
        return false;
    }
...
MONGO_FP_DECLARE(IMWriteConflictException);
MONGO_FP_DECLARE(IMWriteConflictExceptionForReads);
MONGO_FP_DECLARE(IMPausePrimaryOplogDurabilityLoop);

const std::string kInMemoryEngineName = "inMemory";

class InMemoryRecordStore::OplogStones::InsertChange final : public RecoveryUnit::Change {
public:
...
class InMemoryRecordStore::OplogStones::TruncateChange final : public RecoveryUnit::Change {
public:
...
InMemoryRecordStore::OplogStones::OplogStones(OperationContext* txn, InMemoryRecordStore* rs)
    : _rs(rs) {
...
bool InMemoryRecordStore::OplogStones::isDead() {
    stdx::lock_guard<stdx::mutex> lk(_oplogReclaimMutex);
    return _isDead;
}

void InMemoryRecordStore::OplogStones::kill() {
    {
        stdx::lock_guard<stdx::mutex> lk(_oplogReclaimMutex);
        _isDead = true;
    }
...
void InMemoryRecordStore::OplogStones::awaitHasExcessStonesOrDead() {
...
boost::optional<InMemoryRecordStore::OplogStones::Stone>
InMemoryRecordStore::OplogStones::peekOldestStoneIfNeeded() const {
    stdx::lock_guard<stdx::mutex> lk(_mutex);
...
void InMemoryRecordStore::OplogStones::popOldestStone() {
    stdx::lock_guard<stdx::mutex> lk(_mutex);
    _stones.pop_front();
}

void InMemoryRecordStore::OplogStones::createNewStoneIfNeeded(RecordId lastRecord) {
...
void InMemoryRecordStore::OplogStones::updateCurrentStoneAfterInsertOnCommit(
...
void InMemoryRecordStore::OplogStones::clearStonesOnCommit(OperationContext* txn) {
    txn->recoveryUnit()->registerChange(new TruncateChange(this));
}

void InMemoryRecordStore::OplogStones::updateStonesAfterCappedTruncateAfter(
...
void InMemoryRecordStore::OplogStones::setMinBytesPerStone(int64_t size) {
    invariant(size > 0);
...
void InMemoryRecordStore::OplogStones::setNumStonesToKeep(size_t numStones) {
...
void InMemoryRecordStore::OplogStones::_calculateStones(OperationContext* txn) {
...
void InMemoryRecordStore::OplogStones::_calculateStonesByScanning(OperationContext* txn) {
...
void InMemoryRecordStore::OplogStones::_calculateStonesBySampling(OperationContext* txn,
...
void InMemoryRecordStore::OplogStones::_pokeReclaimThreadIfNeeded() {
    if (hasExcessStones()) {
        _oplogReclaimCv.notify_one();
    }
}
...
public:
    Cursor(OperationContext* txn, const InMemoryRecordStore& rs, bool forward = true)
        : _rs(rs),
          _txn(txn),
          _forward(forward),
          _readUntilForOplog(InMemoryRecoveryUnit::get(txn)->getOplogReadTill()) {
    ...
    bool restore() final {
        ...
        invariant(InMemoryRecoveryUnit::get(_txn)->getSession(_txn) == _cursor->getSession());
        _skipNextAdvance = false;
...
private:
    ...
    boost::optional<InMemoryCursor> _cursor;
    bool _eof = false;
    RecordId _lastReturnedId;  // If null, need to seek to first/last record.
    const RecordId _readUntilForOplog;
};

StatusWith<std::string> InMemoryRecordStore::parseOptionsField(const BSONObj options) {
    StringBuilder ss;
    BSONForEach(elem, options) {
        if (elem.fieldNameStringData() == "configString") {
            Status status = InMemoryUtil::checkTableCreationOptions(elem);
            if (!status.isOK()) {
                return status;
            }
...
class InMemoryRecordStore::RandomCursor final : public RecordCursor {
public:
    RandomCursor(OperationContext* txn, const InMemoryRecordStore& rs, StringData config)
    ...
    bool restore() final {
        ...
        WT_SESSION* session = InMemoryRecoveryUnit::get(_txn)->getSession(_txn)->getSession();
...
// static
StatusWith<std::string> InMemoryRecordStore::generateCreateString(
    ...
    ss << "checksum=on,";
    if (inMemoryGlobalOptions.useCollectionPrefixCompression) {
        ss << "prefix_compression,";
    }

    ss << "block_compressor=" << inMemoryGlobalOptions.collectionBlockCompressor << ",";

    ss << InMemoryCustomizationHooks::get(getGlobalServiceContext())->getOpenConfig(ns);
...
InMemoryRecordStore::InMemoryRecordStore(OperationContext* ctx,
                                             StringData ns,
                                             StringData uri,
                                             ...
                                             InMemorySizeStorer* sizeStorer)
    : RecordStore(ns),
      _uri(uri.toString()),
      _tableId(InMemorySession::genTableId()),
      ...
      _shuttingDown(false) {
    Status versionStatus = InMemoryUtil::checkApplicationMetadataFormatVersion(
        ctx, uri, kMinimumRecordStoreVersion, kMaximumRecordStoreVersion);
    if (!versionStatus.isOK()) {
        if (versionStatus.code() == ErrorCodes::FailedToParse) {
            uasserted(58548, versionStatus.reason());
        } else {
            fassertFailedNoTrace(64433);
        }
    }
    ...
    if (InMemoryKVEngine::initRsOplogBackgroundThread(ns)) {
        _oplogStones = std::make_shared<OplogStones>(ctx, this);
    }

    if (_isOplog) {
        _oplogJournalThread = stdx::thread(&InMemoryRecordStore::_oplogJournalThreadLoop,
                                           this,
                                           InMemoryRecoveryUnit::get(ctx)->getSessionCache());
    }
}

InMemoryRecordStore::~InMemoryRecordStore() {
    {
    ...
    LOG(1) << "~InMemoryRecordStore for: " << ns();
    if (_sizeStorer) {
...
bool InMemoryRecordStore::inShutdown() const {
    stdx::lock_guard<boost::timed_mutex> lk(_cappedDeleterMutex);  // NOLINT
    return _shuttingDown;
}

long long InMemoryRecordStore::dataSize(OperationContext* txn) const {
    return _dataSize.load();
}

long long InMemoryRecordStore::numRecords(OperationContext* txn) const {
    return _numRecords.load();
}

bool InMemoryRecordStore::isCapped() const {
    return _isCapped;
}

int64_t InMemoryRecordStore::cappedMaxDocs() const {
    invariant(_isCapped);
    return _cappedMaxDocs;
}

int64_t InMemoryRecordStore::cappedMaxSize() const {
    invariant(_isCapped);
...
int64_t InMemoryRecordStore::storageSize(OperationContext* txn,
    ...
    InMemorySession* session = InMemoryRecoveryUnit::get(txn)->getSession(txn);
    StatusWith<int64_t> result =
        InMemoryUtil::getStatisticsValueAs<int64_t>(session->getSession(),
                                                      "statistics:" + getURI(),
                                                      "statistics=(size)",
...
RecordData InMemoryRecordStore::_getData(const InMemoryCursor& cursor) const {
    WT_ITEM value;
...
RecordData InMemoryRecordStore::dataFor(OperationContext* txn, const RecordId& id) const {
    // ownership passes to the shared_array created below
    InMemoryCursor curwrap(_uri, _tableId, true, txn);
    WT_CURSOR* c = curwrap.get();
    invariant(c);
    c->set_key(c, _makeKey(id));
    int ret = WT_READ_CHECK(c->search(c));
    massert(58556, "Didn't find RecordId in InMemoryRecordStore", ret != WT_NOTFOUND);
    invariantWTOK(ret);
...
void InMemoryRecordStore::deleteRecord(OperationContext* txn, const RecordId& id) {
    // Deletes should never occur on a capped collection because truncation uses
    // WT_SESSION::truncate().
    invariant(!isCapped());

    InMemoryCursor cursor(_uri, _tableId, true, txn);
...
bool InMemoryRecordStore::cappedAndNeedDelete() const {
    if (!_isCapped)
        return false;
...
int64_t InMemoryRecordStore::cappedDeleteAsNeeded(OperationContext* txn,
...
int64_t InMemoryRecordStore::cappedDeleteAsNeeded_inlock(OperationContext* txn,
                                                           const RecordId& justInserted) {
    // we do this in a side transaction in case it aborts
    InMemoryRecoveryUnit* realRecoveryUnit =
        checked_cast<InMemoryRecoveryUnit*>(txn->releaseRecoveryUnit());
    invariant(realRecoveryUnit);
    InMemorySessionCache* sc = realRecoveryUnit->getSessionCache();
    OperationContext::RecoveryUnitState const realRUstate =
        txn->setRecoveryUnit(new InMemoryRecoveryUnit(sc), OperationContext::kNotInUnitOfWork);

    WT_SESSION* session = InMemoryRecoveryUnit::get(txn)->getSession(txn)->getSession();
    ...
    try {
        ...
        if (docsRemoved > 0) {
            ...
            InMemoryCursor startWrap(_uri, _tableId, true, txn);
...
bool InMemoryRecordStore::yieldAndAwaitOplogDeletionRequest(OperationContext* txn) {
...
void InMemoryRecordStore::reclaimOplog(OperationContext* txn) {
    while (auto stone = _oplogStones->peekOldestStoneIfNeeded()) {
        ...
        InMemoryRecoveryUnit* ru = InMemoryRecoveryUnit::get(txn);
        ...
        try {
            ...
            InMemoryCursor endwrap(_uri, _tableId, true, txn);
...
Status InMemoryRecordStore::insertRecords(OperationContext* txn,
    ...
    for (auto& record : *records) {
        ...
        InMemoryItem value(record.data.data(), record.data.size());
        c->set_value(c, value.Get());
        int ret = WT_OP_CHECK(c->insert(c));
        if (ret)
            return wtRCToStatus(ret, "InMemoryRecordStore::insertRecord");
    }
...
void InMemoryRecordStore::_dealtWithCappedId(SortedRecordIds::iterator it, bool didCommit) {
...
bool InMemoryRecordStore::isCappedHidden(const RecordId& id) const {
    stdx::lock_guard<stdx::mutex> lk(_uncommittedRecordIdsMutex);
...
RecordId InMemoryRecordStore::lowestCappedHiddenRecord() const {
    stdx::lock_guard<stdx::mutex> lk(_uncommittedRecordIdsMutex);
...
void InMemoryRecordStore::_oplogSetStartHack(InMemoryRecoveryUnit* wru) const {
    stdx::lock_guard<stdx::mutex> lk(_uncommittedRecordIdsMutex);
    if (_uncommittedRecordIds.empty()) {
...
std::unique_ptr<SeekableRecordCursor> InMemoryRecordStore::getCursor(OperationContext* txn,
                                                                       bool forward) const {
    if (_isOplog && forward) {
        InMemoryRecoveryUnit* wru = InMemoryRecoveryUnit::get(txn);
...
std::unique_ptr<RecordCursor> InMemoryRecordStore::getRandomCursor(OperationContext* txn) const {
    const char* extraConfig = "";
    return getRandomCursorWithOptions(txn, extraConfig);
}

std::unique_ptr<RecordCursor> InMemoryRecordStore::getRandomCursorWithOptions(
    OperationContext* txn, StringData extraConfig) const {
    return stdx::make_unique<RandomCursor>(txn, *this, extraConfig);
}

std::vector<std::unique_ptr<RecordCursor>> InMemoryRecordStore::getManyCursors(
...
Status InMemoryRecordStore::compact(OperationContext* txn,
                                      ...
                                      CompactStats* stats) {
    InMemorySessionCache* cache = InMemoryRecoveryUnit::get(txn)->getSessionCache();
    if (!cache->isEphemeral()) {
        InMemorySession* session = cache->getSession();
...
Status InMemoryRecordStore::validate(OperationContext* txn,
    ...
    if (!_isEphemeral) {
        int err = InMemoryUtil::verifyTable(txn, _uri, &results->errors);
        if (err == EBUSY) {
...
void InMemoryRecordStore::appendCustomStats(OperationContext* txn,
    ...
    BSONObjBuilder bob(result->subobjStart(_engineName));
    {
        ...
        Status status = InMemoryUtil::getApplicationMetadata(txn, getURI(), &metadata);
        if (!status.isOK()) {
    ...
    InMemoryUtil::fetchTypeAndSourceURI(txn, _uri, &type, &sourceURI);
    StatusWith<std::string> metadataResult = InMemoryUtil::getMetadata(txn, sourceURI);
    ...
    Status status =
        InMemoryUtil::exportTableToBSON(s, "statistics:" + getURI(), "statistics=(fast)", &bob);
    if (!status.isOK()) {
...
Status InMemoryRecordStore::oplogDiskLocRegister(OperationContext* txn, const Timestamp& opTime) {
...
class InMemoryRecordStore::CappedInsertChange : public RecoveryUnit::Change {
public:
    CappedInsertChange(InMemoryRecordStore* rs, SortedRecordIds::iterator it)
        : _rs(rs), _it(it) {}
...
private:
    InMemoryRecordStore* const _rs;
    const SortedRecordIds::iterator _it;
};

void InMemoryRecordStore::_oplogJournalThreadLoop(InMemorySessionCache* sessionCache) try {
    ...
    while (true) {
        ...
        while (!_shuttingDown && MONGO_FAIL_POINT(IMPausePrimaryOplogDurabilityLoop)) {
            lk.unlock();
            sleepmillis(10);
            lk.lock();
        }
...
void InMemoryRecordStore::waitForAllEarlierOplogWritesToBeVisible(OperationContext* txn) const {
...
void InMemoryRecordStore::_addUncommittedRecordId_inlock(OperationContext* txn, RecordId id) {
...
void InMemoryRecordStore::updateStatsAfterRepair(OperationContext* txn,
                                                   long long numRecords,
                                                   long long dataSize) {
    _numRecords.store(numRecords);
    _dataSize.store(dataSize);
...
RecordId InMemoryRecordStore::_nextId() {
...
InMemoryRecoveryUnit* InMemoryRecordStore::_getRecoveryUnit(OperationContext* txn) {
    return checked_cast<InMemoryRecoveryUnit*>(txn->recoveryUnit());
}

class InMemoryRecordStore::NumRecordsChange : public RecoveryUnit::Change {
public:
    NumRecordsChange(InMemoryRecordStore* rs, int64_t diff) : _rs(rs), _diff(diff) {}
    virtual void commit() {}
    virtual void rollback() {
...
private:
    InMemoryRecordStore* _rs;
    int64_t _diff;
};

void InMemoryRecordStore::_changeNumRecords(OperationContext* txn, int64_t diff) {
...
class InMemoryRecordStore::DataSizeChange : public RecoveryUnit::Change {
public:
    DataSizeChange(InMemoryRecordStore* rs, int64_t amount) : _rs(rs), _amount(amount) {}
    virtual void commit() {}
    virtual void rollback() {
...
void InMemoryRecordStore::_increaseDataSize(OperationContext* txn, int64_t amount) {
    if (txn)
...
int64_t InMemoryRecordStore::_makeKey(const RecordId& id) {
    return id.repr();
}
RecordId InMemoryRecordStore::_fromKey(int64_t key) {
    return RecordId(key);
}
...
void InMemoryRecordStore::temp_cappedTruncateAfter(OperationContext* txn,
    ...
    massert(58807, str::stream() << "Failed to seek to the record located at " << end, record);
