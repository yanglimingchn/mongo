...
public:
    ...
    InMemoryHarnessHelper()
        : _dbpath("wt_test"),
          ...
          _sessionCache(new InMemorySessionCache(_conn)) {}

    InMemoryHarnessHelper(StringData extraStrings)
        : _dbpath("wt_test"),
    ...
    std::unique_ptr<RecordStore> newNonCappedRecordStore(const std::string& ns) {
        InMemoryRecoveryUnit* ru = new InMemoryRecoveryUnit(_sessionCache);
        OperationContextNoop txn(ru);
        string uri = "table:" + ns;

        StatusWith<std::string> result = InMemoryRecordStore::generateCreateString(
            kInMemoryEngineName, ns, CollectionOptions(), "");
        ...
        return stdx::make_unique<InMemoryRecordStore>(
            &txn, ns, uri, kInMemoryEngineName, false, false);
    }
    ...
    std::unique_ptr<RecordStore> newCappedRecordStore(const std::string& ns,
        ...
        StatusWith<std::string> result =
            InMemoryRecordStore::generateCreateString(kInMemoryEngineName, ns, options, "");
        ASSERT_TRUE(result.isOK());
        std::string config = result.getValue();
        ...
        return stdx::make_unique<InMemoryRecordStore>(
            &txn, ns, uri, kInMemoryEngineName, true, false, cappedMaxSize, cappedMaxDocs);
    }

    RecoveryUnit* newRecoveryUnit() final {
        return new InMemoryRecoveryUnit(_sessionCache);
    }
...
TEST(InMemoryRecordStoreTest, GenerateCreateStringEmptyDocument) {
    BSONObj spec = fromjson("{}");
    StatusWith<std::string> result = InMemoryRecordStore::parseOptionsField(spec);
    ASSERT_OK(result.getStatus());
    ASSERT_EQ(result.getValue(), "");  // "," would also be valid.
}

TEST(InMemoryRecordStoreTest, GenerateCreateStringUnknownField) {
...
TEST(InMemoryRecordStoreTest, GenerateCreateStringNonStringConfig) {
...
TEST(InMemoryRecordStoreTest, GenerateCreateStringEmptyConfigString) {
...
TEST(InMemoryRecordStoreTest, GenerateCreateStringInvalidConfigStringOption) {
    BSONObj spec = fromjson("{configString: 'abc=def'}");
    ASSERT_EQ(InMemoryRecordStore::parseOptionsField(spec), ErrorCodes::BadValue);
}

TEST(InMemoryRecordStoreTest, GenerateCreateStringValidConfigStringOption) {
    BSONObj spec = fromjson("{configString: 'prefix_compression=true'}");
    ASSERT_EQ(InMemoryRecordStore::parseOptionsField(spec),
              std::string("prefix_compression=true,"));
}

TEST(InMemoryRecordStoreTest, Isolation1) {
...
TEST(InMemoryRecordStoreTest, Isolation2) {
    unique_ptr<HarnessHelper> harnessHelper(newHarnessHelper());
    unique_ptr<RecordStore> rs(harnessHelper->newNonCappedRecordStore());
...
TEST(InMemoryRecordStoreTest, SizeStorer1) {
    unique_ptr<InMemoryHarnessHelper> harnessHelper(new InMemoryHarnessHelper());
    unique_ptr<RecordStore> rs(harnessHelper->newNonCappedRecordStore());

    string uri = checked_cast<InMemoryRecordStore*>(rs.get())->getURI();

    string indexUri = "table:myindex";
    InMemorySizeStorer ss(harnessHelper->conn(), indexUri);
    checked_cast<InMemoryRecordStore*>(rs.get())->setSizeStorer(&ss);
    ...
    {
        unique_ptr<OperationContext> opCtx(harnessHelper->newOperationContext());
        rs.reset(new InMemoryRecordStore(
            opCtx.get(), "a.b", uri, kInMemoryEngineName, false, false, -1, -1, NULL, &ss));
    }
    ...
    {
        unique_ptr<OperationContext> opCtx(harnessHelper->newOperationContext());
        InMemoryRecoveryUnit* ru = checked_cast<InMemoryRecoveryUnit*>(opCtx->recoveryUnit());
    ...
    {
        unique_ptr<OperationContext> opCtx(harnessHelper->newOperationContext());
        InMemorySizeStorer ss2(harnessHelper->conn(), indexUri);
        ss2.fillCache();
        long long numRecords;
        long long dataSize;
...
private:
    virtual void setUp() {
        harnessHelper.reset(new InMemoryHarnessHelper());
        sizeStorer.reset(new InMemorySizeStorer(harnessHelper->conn(), "table:sizeStorer"));
        rs = harnessHelper->newNonCappedRecordStore();
        InMemoryRecordStore* wtrs = checked_cast<InMemoryRecordStore*>(rs.get());
...
protected:
    ...
    std::unique_ptr<InMemoryHarnessHelper> harnessHelper;
    std::unique_ptr<InMemorySizeStorer> sizeStorer;
...
TEST_F(SizeStorerValidateTest, InvalidSizeStorerAtCreation) {
    ...
    rs.reset(new InMemoryRecordStore(opCtx.get(),
                                       "a.b",
                                       uri,
                                       kInMemoryEngineName,
                                       false,
...
StatusWith<RecordId> insertBSON(unique_ptr<OperationContext>& opCtx,
    ...
    InMemoryRecordStore* wrs = checked_cast<InMemoryRecordStore*>(rs.get());
    invariant(wrs);
...
// TODO make generic
TEST(InMemoryRecordStoreTest, OplogHack) {
    InMemoryHarnessHelper harnessHelper;
...
TEST(InMemoryRecordStoreTest, OplogHackOnNonOplog) {
    InMemoryHarnessHelper harnessHelper;
...
TEST(InMemoryRecordStoreTest, CappedOrder) {
    unique_ptr<InMemoryHarnessHelper> harnessHelper(new InMemoryHarnessHelper());
...
TEST(InMemoryRecordStoreTest, CappedCursorRollover) {
    unique_ptr<InMemoryHarnessHelper> harnessHelper(new InMemoryHarnessHelper());
...
TEST(InMemoryRecordStoreTest, OplogOrder) {
    unique_ptr<InMemoryHarnessHelper> harnessHelper(new InMemoryHarnessHelper());
    unique_ptr<RecordStore> rs(harnessHelper->newCappedRecordStore("local.oplog.foo", 100000, -1));

    {
        const InMemoryRecordStore* wrs = checked_cast<InMemoryRecordStore*>(rs.get());
        ASSERT(wrs->isOplog());
...
TEST(InMemoryRecordStoreTest, OplogDurableVisibilityInOrder) {
    ON_BLOCK_EXIT([] { IMPausePrimaryOplogDurabilityLoop.setMode(FailPoint::off); });
    IMPausePrimaryOplogDurabilityLoop.setMode(FailPoint::alwaysOn);

    unique_ptr<InMemoryHarnessHelper> harnessHelper(new InMemoryHarnessHelper());
    unique_ptr<RecordStore> rs(harnessHelper->newCappedRecordStore("local.oplog.foo", 100000, -1));
    auto wtrs = checked_cast<InMemoryRecordStore*>(rs.get());
...
TEST(InMemoryRecordStoreTest, OplogDurableVisibilityOutOfOrder) {
    ON_BLOCK_EXIT([] { IMPausePrimaryOplogDurabilityLoop.setMode(FailPoint::off); });
    IMPausePrimaryOplogDurabilityLoop.setMode(FailPoint::alwaysOn);
    ...
    IMPausePrimaryOplogDurabilityLoop.setMode(FailPoint::off);
...
TEST(InMemoryRecordStoreTest, StorageSizeStatisticsDisabled) {
    InMemoryHarnessHelper harnessHelper("statistics=(none)");
...
TEST(InMemoryRecordStoreTest, AppendCustomStatsMetadata) {
    InMemoryHarnessHelper harnessHelper;
    unique_ptr<RecordStore> rs(harnessHelper.newNonCappedRecordStore("a.b"));
    ...
    BSONElement inMemoryElement = customStats.getField(kInMemoryEngineName);
    ASSERT_TRUE(inMemoryElement.isABSONObj());
    BSONObj inMemory = inMemoryElement.Obj();

    BSONElement metadataElement = inMemory.getField("metadata");
    ...
    BSONElement creationStringElement = inMemory.getField("creationString");
    ASSERT_EQUALS(creationStringElement.type(), String);
}

TEST(InMemoryRecordStoreTest, CappedCursorYieldFirst) {
    unique_ptr<InMemoryHarnessHelper> harnessHelper(new InMemoryHarnessHelper());
...
StatusWith<RecordId> insertBSONWithSize(OperationContext* opCtx,
    ...
    InMemoryRecordStore* wtrs = checked_cast<InMemoryRecordStore*>(rs);
    invariant(wtrs);
...
TEST(InMemoryRecordStoreTest, OplogStones_CreateNewStone) {
    InMemoryHarnessHelper harnessHelper;
    ...
    InMemoryRecordStore* wtrs = static_cast<InMemoryRecordStore*>(rs.get());
    InMemoryRecordStore::OplogStones* oplogStones = wtrs->oplogStones();
...
// record is changed.
TEST(InMemoryRecordStoreTest, OplogStones_UpdateRecord) {
    InMemoryHarnessHelper harnessHelper;
...
TEST(InMemoryRecordStoreTest, OplogStones_Truncate) {
    InMemoryHarnessHelper harnessHelper;
...
TEST(InMemoryRecordStoreTest, OplogStones_CappedTruncateAfter) {
    InMemoryHarnessHelper harnessHelper;
...
TEST(InMemoryRecordStoreTest, OplogStones_ReclaimStones) {
    InMemoryHarnessHelper harnessHelper;
...
// 'cappedMaxSize'.
TEST(InMemoryRecordStoreTest, OplogStones_ExceedCappedMaxSize) {
    InMemoryHarnessHelper harnessHelper;
...
TEST(InMemoryRecordStoreTest, OplogStones_AscendingOrder) {
    InMemoryHarnessHelper harnessHelper;
